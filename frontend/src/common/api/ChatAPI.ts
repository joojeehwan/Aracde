...
const getChatList = async () => {
  ...
  if (token !== null) {
    const result = await axios.get(`${BASE_URL}`, { headers: { Authorization: token } })
      .then((res) => {
        console.log(res)
        return res
      })
...
const getChatSearchResult = async (name: string) => {
  const token = getToken()
  if (token !== null) {
    const result = await axios.get(`${BASE_URL}/search?name=${name}`, { headers: { Authorization: token } })
      .then((res) => {
        console.log(res)
        return res
      })
...
const createChatRoom = async (targetUserSeq: any) => {
  ...
  if (token !== null) {
    const result = await axios.post(`${BASE_URL}/create`, targetUserSeq, { headers: { Authorization: token } })
      .then((res) => {
        console.log(res);
      })
      .catch((err) => {
